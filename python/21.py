...
monkey_commands = {(data_split:=line.split(': '))[0]: command.split(' ')
                    if len((command:=data_split[1])) > 4 else float(command)
                    for line in data}
...
def yell(monkey_commands, which='root'):
    ...
    while commands:
        ...
        if not isinstance(monkey_commands[monkey], float):
            left, operation, right = command
            if isinstance(monkey_commands[left], float) and isinstance(monkey_commands[right], float):
...
party_1 = int(yell(dict(monkey_commands)))


# pt 2 binary search by hand haha
monkey_commands['humn'] = float(3343167719435)
first, _, second = monkey_commands['root']
monkey_commands['root'] = (first, '-', second)
if yell(monkey_commands) == 0.0:
    party_2 = int(monkey_commands['humn'])
